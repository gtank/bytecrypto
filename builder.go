...
// highest byte of the 32-bit input value is silently truncated.
...
func (b *Builder) addLengthPrefixed(lenLen int, isASN1 bool, f BuilderContinuation) {
	...
	b.flushChild()
	if b.child != nil {
		panic("bytecrypto: internal error")
	}
}
...
func (b *Builder) flushChild() {
	...
	if length < 0 {
		panic("bytecrypto: internal error") // result unexpectedly shrunk
	}
	...
	if l != 0 {
		b.err = fmt.Errorf("bytecrypto: pending child length %d exceeds %d-byte length prefix", length, child.pendingLenLen)
		return
	}
...
func (b *Builder) add(bytes ...byte) {
	...
	if len(b.result)+len(bytes) < len(bytes) {
		b.err = errors.New("bytecrypto: length overflow")
	}
	if b.fixedSize && len(b.result)+len(bytes) > cap(b.result) {
		b.err = errors.New("bytecrypto: Builder is exceeding its fixed-size buffer")
		return
	}
